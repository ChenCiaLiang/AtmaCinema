...
class UserClient {
  ...
  static Future<Response> register(User user) async {
    try {
      var response = await post(
          Uri.parse('${apibro.url}/register'), // pergi ke /api/register
  ...
  static Future<http.Response> update(User user, {File? profileImage}) async {
    try {
      ...
      if (token != null) {
        var uri = Uri.parse('${apibro.url}/update/$userId');
  ...
  static Future<Response> destroy(id) async {
    try {
      var response =
          await delete(Uri.parse('${apibro.url}/$id'));
